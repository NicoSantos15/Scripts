...
const CustomColorDialog = Java.type(
  "com.archimatetool.editor.ui.components.CustomColorDialog"
);
const ColorMapWizardPage = Java.extend(
  Java.type("org.eclipse.jface.wizard.WizardPage")
);
  ...
  createControl(parent) {
    try {
      ...
      list.addMouseListener(
        MouseListener.mouseDoubleClickAdapter((e) => {
          try {
            // going next page
      ...
      pagePropertySelection.setDescription(
        "Among the properties found in current view to be used to colourise the elements"
      );
      pagePropertySelection.setControl(container);
      // pagePropertySelection.setPageComplete(false); // true only after selection
      pagePropertySelection.setPageComplete(Wizard.cm.hasProperty); // should never be false
      log.trace(` ... created`);
    } catch (error) {
  ...
  createControl: function (parent) {
    try {
        ...
        return MouseListener.mouseDoubleClickAdapter((e) => {
          try {
            pageLabelsSelection
              .getWizard()
              .getContainer()
              .showPage(pageLabelsSelection.getNextPage());
          } catch (err) {
            log.error(err.toString());
          }
        });
      ...
      GridLayoutFactory.swtDefaults()
        ...
        .margins(20, 10)
        .spacing(20, 10)
        .applyTo(container); // spacing(10, 5).
      WidgetFactory.label(SWT.NONE)
        .text("Labels:")
        .layoutData(GridDataFactory.fillDefaults().create())
        .create(container);

      Wizard.labelsTable = new Table(
        container,
        SWT.CHECK | SWT.BORDER | SWT.V_SCROLL | SWT.HIDE_SELECTION
      );
      ...
      Wizard.labelsTable.addSelectionListener(
        SelectionListener.widgetSelectedAdapter((e) => {
          try {
            if (e.detail == SWT.CHECK) {
              if (e.item == Wizard.allLabelsCheckbox) {
                const state = Wizard.allLabelsCheckbox.getChecked();
                Wizard.cm.colormap.setSelection(
                  Wizard.cm.colormap.labels(),
                  state
                );
              } else {
                Wizard.cm.colormap.setSelection(
      ...
      WidgetFactory.label(SWT.NONE).text("Color scale:").create(container);
      ...
      Wizard.btnContinuous = WidgetFactory.button(SWT.RADIO)
        ...
        .tooltip(
          "Use a continuous (gradient) color scheme (for numeric labels only)"
        )
      ...
      WidgetFactory.label(SWT.NONE)
        .text(
          "You can choose a continuous scale" +
      ...
      pageLabelsSelection.setDescription(
        "Select the property values to colourise, and the type of color scale."
      );
  ...
  setVisible: function (visible) {
    ...
    function updateLabelCheckMarks(colormap) {
      try {
        assert(
          colormap != undefined,
          "Colormap undefined when calling updateLabelCheckMarks"
        );
        ...
        for (const i of table.getItems()) {
          ...
          if (updated.includes(label)) {
            log.trace(
              `Syncing '${label}' checkbox with ${JSON.stringify(
                colormap.get(label)
              )} `
            );
    ...
    try {
      if (visible) {
        ...
        pageLabelsSelection.setTitle(
          `Labels selection for '${Wizard.cm.property}'`
        );
      ...
      log.info(
        err.toString(),
        ":",
        Java.isJavaObject(err) ? Java.typeName(err) : typeof err
      );
    } finally {
...
function createColorSchemeWidgets(container, verticalSpan) {
  ...
  const group = WidgetFactory.group(SWT.NONE)
    .text("Color scheme")
    .layout(layout)
    .create(container);
  ...
  WidgetFactory.button(SWT.PUSH)
    ...
    .layoutData(GridDataFactory.swtDefaults().create())
    .onSelect((_) => {
      Wizard.cm.colormap.loadColorScheme();
    })
    .create(group);
...
const pageCategoryColor = new ColorMapWizardPage("pageCategoryColor", {
  createControl: function (parent) {
    try {
        ...
        if (Wizard.catColorTable.getSelectionCount() == 0) {
          ...
          try {
            ...
            if (selectedColor) {
              ...
              const labels = Java.from(Wizard.catColorTable.getSelection()).map(
                (item) => item.getText()
              );
      ...
      WidgetFactory.label(SWT.NONE)
        .text(
          'Select one or more labels, double-click or click "Set Color" to define the color'
        )
        .layoutData(GridDataFactory.fillDefaults().span(2, 1).create())
        .create(container);

      Wizard.catColorTable = WidgetFactory.table(
        SWT.BORDER | SWT.MULTI | SWT.FULL_SELECTION
      ).create(container);
      GridDataFactory.defaultsFor(Wizard.catColorTable)
        .align(SWT.FILL, SWT.FILL)
        .grab(true, true)
        .span(1, 2)
        .applyTo(Wizard.catColorTable);
      Wizard.catColorTable.addMouseListener(
        MouseListener.mouseDoubleClickAdapter(setColor)
      );
  ...
  setVisible: function (visible) {
    ...
    function updateColorImages(colormap) {
      assert(
        colormap != undefined,
        "Colormap undefined when calling updateColorImages"
      );
      for (const item of Wizard.catColorTable.getItems()) {
        const label = item.getText();
        if (colormap.has(label)) {
          log.debug(
            `callback: setting color ${item.getData().color} to ${label}`
          );
  ...
  createControl(parent) {
    try {
      ...
      function setColor(event) {
        ...
        try {
          ...
          if (selectedColor) {
            const hexColor = ImageRegistry.swtRGBToHex(selectedColor);
            log.info(
              `Setting color ${hexColor} for button #${event.source.getData()}`
            );
            if (Wizard.cm.colormap.scale instanceof ContinuousScale)
              Wizard.cm.colormap.scale.setGradientColor(
                hexColor,
                event.source.getData()
              );
            else
      ...
      WidgetFactory.label(SWT.CENTER)
        .text("Start")
        .layoutData(
          GridDataFactory.swtDefaults().align(SWT.CENTER, SWT.END).create()
        )
        .create(container);

      Wizard.startValue = WidgetFactory.label(SWT.LEFT)
        .text("")
        .layoutData(GridDataFactory.fillDefaults().grab(true, false).create())
        .create(container);

      Wizard.endValue = WidgetFactory.label(SWT.RIGHT)
        .text("")
      ...
      // Gradient management
      Wizard.startBtn = WidgetFactory.button(SWT.PUSH)
      ...
      Wizard.gradientLabel = WidgetFactory.label(SWT.BORDER)
        .tooltip("click to set middle color position, if enabled")
        .layoutData(
          GridDataFactory.fillDefaults().span(2, 1).grab(true, false).create()
        )
        .create(container);

      Wizard.gradientLabel.addMouseListener(
        MouseListener.mouseDownAdapter((e) => {
          try {
            log.trace(
              `Click received ${
                e.button == 1 ? "Left" : e.button == 2 ? "Right" : "Other"
              }-click on ${e.x},${e.y}`
            );
            const scale = Wizard.cm.colormap.scale;
            if (scale instanceof ContinuousScale) {
              /// syntax checking purpose
      ...
      Wizard.gradientLabel.addControlListener(
        ControlListener.controlResizedAdapter((e) => {
          try {
            ...
            log.debug(
              `gradientLabel resized: ${bounds.width}x${bounds.height}`
            );
            const scale = Wizard.cm.colormap.scale;
            if (scale instanceof ContinuousScale) {
              Wizard.gradientLabel.setImage(
                Wizard.ir.getGradientImage(scale.relativeEdges(), bounds)
              );
            }
          } catch (err) {
            log.error(err.toString());
      ...
      Wizard.endBtn = WidgetFactory.button(SWT.PUSH)
      ...
      middleGroup.setLayoutData(
        GridDataFactory.swtDefaults().span(4, 1).create()
      );

      Wizard.cbMiddleColor = WidgetFactory.button(SWT.CHECK)
        .tooltip(`Add a middle color definition`)
        .text("Middle color")
        .onSelect((e) => {
          if (Wizard.cm.colormap.scale instanceof ContinuousScale) {
            Wizard.cm.colormap.scale.enableMiddleColor(
              Wizard.cbMiddleColor.getSelection()
            );
          }
        })
        .create(middleGroup);
      ...
      Wizard.middleBtn = WidgetFactory.button(SWT.PUSH)
        .image(Wizard.ir.unknownImage)
      ...
      Wizard.middlePosition = WidgetFactory.spinner(SWT.BORDER)
        .tooltip("Also click on gradient bar to set position")
        .onSelect((e) => {
          const scale = Wizard.cm.colormap.scale;
          if (scale instanceof ContinuousScale) {
            ...
            log.trace(
              `Setting middle position ${spinner.getSelection()} (=> ${position})`
            );
      ...
      Wizard.gradientTable = WidgetFactory.table(
      ...
      pageContinuousColor.setDescription(
        `Select start, end and optionally middle colors to define gradient`
      );
      log.trace("... created");
    } catch (err) {
      log.error(err.toString());
  ...
  setVisible: function (visible) {
    ...
    function updateGradientColors(colormap) {
      ...
      if (scale instanceof ContinuousScale) {
        ...
        Wizard.middleBtn.setImage(
          Wizard.ir.getImage(
            scale.isMiddleColorEnabled() ? scale.middle.color : undefined
          )
        );
        ...
        const pos = scale.isMiddleColorEnabled()
          ? Math.round(scale.middle.textAsNumber * factor)
          : 0;
    ...
    try {
      if (visible) {
        ...
        if (scale instanceof ContinuousScale) {
          ...
          const spinner = Wizard.middlePosition;
          let exp =
            Math.log10(scale.end.textAsNumber - scale.start.textAsNumber) - 2;
          ...
          log.trace(
            `delta = ${Math.log10(
              scale.end.textAsNumber - scale.start.textAsNumber
            )} Exp = ${exp}. Setting Middle spinner digits to ${digits}`
          );
        ...
        pageContinuousColor.setTitle(
          `Continuous color scale for '${Wizard.cm.property}'`
        );
      } else {
...
const WIZARD_SUBCLASS_EXTENSION = {
  ...
  canFinish: function () {
    const result =
      Wizard.cm.hasProperty && // has property selected
      Wizard.cm.colormap.isApplicable() &&
      Wizard.cm.colormap.scale.isDefined();
    return result;
  },
...
const Wizard = {
  ...
  /** @type {JavaObject} */
  btnSave2: undefined,
  element: undefined, // the selected element type
   ...
   */
  execute: function () {
    Wizard.cm = new ColorModel(hardcodedProperties);
    const ColorMapWizard = Java.extend(
      Java.type("org.eclipse.jface.wizard.Wizard")
    );
